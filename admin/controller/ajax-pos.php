    ...
    if(is_array($obj))
    {
        if($obj['data_type'] == "search")
        {
            ...
            if($obj['text']){
                ...
                $barcode = $obj['text'];
                $text = "%".$obj['text']."%";
                $qry = "select * from tblproduct where category in(1,2,3) && description like :find || p_name = :p_name order by p_name asc";
                $row = $productClass->query($qry,['find'=>$text,'p_name'=>$barcode]); 
            }else{
                //get all
                $row = $productClass->query("select * from tblproduct  where category in(1,2,3) order by id asc");
            }
                ...
                if(is_array($check)){
                    ...
                    $arr['trans_count'] = $trans_count;
                     ...
                     $qry = "insert into tblcart(transno,qty,price,total,sdate,stime,user_id,p_id,status,trans_count)values(:transno,:qty,:price,:total,:sdate,:stime,:user_id,:p_id,:status,:trans_count)";
                        ...
                        update tblproduct set qty = qty - '$qty' where id = '$p_id' limit 1");
