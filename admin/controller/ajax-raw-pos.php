        ...
        if($obj['data_type'] == 'checkout')
        {
            ...
            foreach ($data as $row){
                ...
                if(is_array($check)){
                    ...
                    $arr['qty'] = $row['qty'];
                    $arr['cost'] = $check['cost'];
                    $arr['total_raw'] = $row['qty'] * $check['cost'];
                    $arr['transno'] = $date_tr;
                    $arr['sdate'] = $date;
                    $arr['stime'] = $time;
                    $arr['user_id'] = $user_id;
                    ...
                    $qry = "insert into tblcart(transno,qty,cost,total_raw,sdate,stime,user_id,p_id,status,trans_count)values(:transno,:qty,:cost,:total_raw,:sdate,:stime,:user_id,:p_id,:status,:trans_count)";
                    $db->query($qry,$arr);
