...
defined("ABSPATH") ? "" : die();
$_SESSION['revenue'] = "";
$tab = $_GET['pg'] ?? 'daily-record';
    $date = date('Y-m-d');
    $saleClass = new Sales();
    $sales = $saleClass->query("select * from vw_sales where sdate = '$date' && status = 'Sold' &&  category != '4' order by id desc");
...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if($cashier1){
        ...
        $sales = $saleClass->query("select * from vw_sales where (sdate between '$start' and '$end' ) and user_id like '$cashier1' && status = 'Sold' &&  category != '4' order by id desc");
        ...
        $sales = $saleClass->query("select * from vw_sales where (sdate between '$start' and '$end' ) && status = 'Sold' &&  category != '4' order by id desc");
...
require view_path('pos/daily-record');